  ...
  const onBookHandler = async () => {
    ...
    try {
      const res = await axios.post(
        "https://movie-project-backend-ufco.onrender.com/book-ticket",
            ...
            <div className="flex overflow-x-auto whitespace-nowrap gap-3 py-1 px-1 -mx-1 w-full custom-scrollbar">
              {Object.keys(dateTime).map((date) => (
                <button
                  ...
                  key={date}
                  className={`flex flex-col items-center justify-center h-14 w-14 aspect-square rounded flex-shrink-0 transition cursor-pointer text-sm ${
                    selected === date
